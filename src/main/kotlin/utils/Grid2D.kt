...
operator fun <T> Grid2D<T>.get(idx: Point) = get(idx.first, idx.second)
...
val <T> Grid2D<T>.values get() = indices.map { get(it.first, it.second) }
val <T> Grid2D<T>.indexedValues get() = indices.map { it to get(it.first, it.second) }
