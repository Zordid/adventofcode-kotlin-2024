...
abstract class KPixelGameEngine(private val appName: String = "PixelGameEngine") {
    ...
    companion object {
        fun gradientColor(from: Color, to: Color, percent: Double): Color {
            val resultRed = (from.red + percent * (to.red - from.red)).roundToInt()
            val resultGreen = (from.green + percent * (to.green - from.green)).roundToInt()
            val resultBlue = (from.blue + percent * (to.blue - from.blue)).roundToInt()
            return Color(resultRed.coerceIn(0..255), resultGreen.coerceIn(0..255), resultBlue.coerceIn(0..255))
        }

        fun createGradient(from: Color, to: Color = Color.BLACK, steps: Int): List<Color> =
            (0 until steps).map { gradientColor(from, to, it / (steps - 1).toDouble()) }
